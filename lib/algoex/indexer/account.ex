defmodule Algoex.Indexer.Account do
  import Algoex
  ...
  def apps_local_state(account_id, params \\ []) do
    options = [params: params]
    Algoex.get("/v2/accounts/#{account_id}/apps-local-state", [], options)
    |> extract_body_as_map()
  end
  ...
  def assets(account_id, params \\ []) do
    options = [params: params]
    Algoex.get("/v2/accounts/#{account_id}/assets", [], options)
    |> extract_body_as_map()
  end
  ...
  def created_applications(account_id, params \\ []) do
    options = [params: params]
    Algoex.get("/v2/accounts/#{account_id}/created-applications", [], options)
    |> extract_body_as_map()
  end
  ...
  def created_assets(account_id, params \\ []) do
    options = [params: params]
    Algoex.get("/v2/accounts/#{account_id}/created-assets", [], options)
    |> extract_body_as_map()
  end

  def transactions(account_id, params \\ []) do
    options = [params: params]
    Algoex.get("/v2/accounts/#{account_id}/transactions", [], options)
    |> extract_body_as_map()
  end
